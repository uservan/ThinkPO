...
def main():
    ...
    if args.math_difficulty_lower_bound is not None or  args.math_difficulty_upper_bound is not None:
        result_file = os.path.join(args.result_dir, f"{get_MODEL_TO_NAME(args.model)}_{args.dataset}_{args.split}_{args.source}_{args.start}_{args.end}_{args.math_difficulty_lower_bound}_{args.math_difficulty_upper_bound}.json")
    else:
        result_file = os.path.join(args.result_dir, f"{get_MODEL_TO_NAME(args.model)}_{args.dataset}_{args.split}_{args.source}_{args.start}_{args.end}.json")

    if args.check:
        # check if converted file exists
        if args.math_difficulty_lower_bound is not None or args.math_difficulty_upper_bound is not None:
            converted_file = f"{args.result_dir}/converted_{get_MODEL_TO_NAME(args.model)}_{args.dataset}_{args.split}_{args.source}_{args.start}_{args.end}_{args.math_difficulty_lower_bound}_{args.math_difficulty_upper_bound}.json"
        else:
            converted_file = f"{args.result_dir}/converted_{get_MODEL_TO_NAME(args.model)}_{args.dataset}_{args.split}_{args.source}_{args.start}_{args.end}.json"
        ...
        system_prompt = SYSTEM_PROMPT.get(args.model, SYSTEM_PROMPT['NovaSky-AI/Sky-T1-32B-Preview'])
