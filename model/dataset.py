...
class SFTDataset_List(Dataset):
    def __init__(self, list, tokenizer, max_seq_len):
        super().__init__()
        self.list = list
        self.tokenizer = tokenizer
    ...
    def __getitem__(self, index: int):
        example = self.list[index]
        input_ids, attention_mask, labels = [], [], []
        inputs, targets = example["prompt"], example["response"]
        instruction = self.tokenizer(inputs+'\n', add_special_tokens=False)
        response = self.tokenizer(targets, add_special_tokens=False)
        ...
        #     input_ids = input_ids + [self.tokenizer.convert_tokens_to_ids(self.tokenizer.pad_token)] * padding_len
        #     labels = labels + [-100]* padding_len
        #     attention_mask = attention_mask + [0] * padding_len
        return torch.from_numpy(np.array(input_ids)), torch.from_numpy(np.array(labels)), torch.from_numpy(np.array(attention_mask))
